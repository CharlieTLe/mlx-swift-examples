...
A SwiftUI app for macOS, iPhone and iPad that annotates Shakespeare on-device: select
lines in a play and it explains them, in the style of a Genius.com annotation, then
offers four follow-up questions that continue the same conversation about that passage.
After the initial model download there are no network calls.
...
```bash
open mlx-swift-examples.xcodeproj    # scheme: ShakespeareReader
```
...
```bash
xcodebuild -scheme ShakespeareReader -destination 'platform=macOS' build
```
...
build needs none, for the reason the next section gives.

Both builds need the same two things:
...
- **A full Xcode**, selected with `xcode-select`, with the **Metal toolchain**
  ...
  Nothing in the build checks for this, so it is worth checking by hand before the
  first run.
...
**Apple silicon only**, and the project already says so for every target:
`EXCLUDED_ARCHS = x86_64` sits in the project-level build configuration. It is not
belt-and-braces there: `ARCHS_STANDARD` for macosx is still `arm64 x86_64`, and
`ONLY_ACTIVE_ARCH` is Debug-only, so a Release build would otherwise try an x86_64 slice
of the vendored MLX C++ and metal-cpp. The iPhone build never meets this because its
`ARCHS_STANDARD` is arm64 alone.

There is an app icon: a Big Caslon capital `S`, ink on parchment, in
...
the only Caslon macOS ships. The mark is drawn from the glyph's outline rather than laid
out as text, so it can be scaled to an exact cap height and centred on its own bounding
box — centring the typographic box would sit an `S` high in the tile, since it has no
descender. What made a letterform the right answer is the 16pt tile: figure/ground
contrast is the entire design budget there, and a glyph is nothing but one
high-contrast shape. `tools/icon/NOTICE.md` carries the rest of the reasoning.

The 2.2 GB of weights live in `~/.cache/huggingface/hub`, shared with `llm-tool` and
every other example here, and the annotation cache in
`~/Library/Application Support/ShakespeareReader/`. Reading position, `readerFont` and
`showsCommentary` are ordinary preferences under the bundle id:

```bash
defaults read mlx.ShakespeareReader
```

**App Sandbox is deliberately off**, which is the one place this target diverges from
its neighbours, all of which set `ENABLE_APP_SANDBOX = YES`. The Mac build signs against
its own empty `ShakespeareReader-macOS.entitlements` rather than the iPhone file. Sandboxing would
...
Pick your team under **Signing & Capabilities** and Run; `${DISAMBIGUATOR}` derives a
bundle id from it, so there is nothing to edit by hand. One universal build, iOS 18.0 and
up; there is no visionOS layout.

The iPhone entitlements file, `ShakespeareReader-iOS.entitlements`, asks for one
...
- **`FoundationModelsIntegration` is on.** An Xcode project has no way to express a
  package trait, so both destinations compile mlx-swift-lm's `MLXFoundationModels`, which
  that default-on trait gates and which `MLXHuggingFace` pulls in. The app never calls
  into it and the whole target is behind
...
```bash
python3 tools/build_corpus.py --all --out Resources/Plays
...
inside the bundle rather than `open`, and not `./mlx-run ShakespeareReader ...`, which
backgrounds an app scheme with `&` and loses the exit code:

```bash
xcodebuild -scheme ShakespeareReader -destination 'platform=macOS' build
APP=$(xcodebuild -configuration Release -showBuildSettings -scheme ShakespeareReader \
  | sed -n 's/.*BUILT_PRODUCTS_DIR = //p')/ShakespeareReader.app/Contents/MacOS/ShakespeareReader

"$APP" --selftest    # "selftest: all checks passed"; no model, no network
"$APP" --metal-check # "metal: ok"; one array on the GPU, no model, no network
"$APP" --show-prompt # assembled prompts + exact token counts
"$APP" --benchmark   # the latency table above
"$APP" --greedy      # temperature 0, for prompt A/B work
"$APP" --diagnostics # the capsule, the load check and the status strip, this launch only
"$APP" --benchmark --passage hamlet:3.1:62-96
```

The first two are the fast gate: no model, no network, and they are what to run before
any manual pass.
...
- **One multiplatform target**, not one per platform: `SDKROOT = auto`,
  `SUPPORTED_PLATFORMS = "iphoneos iphonesimulator macosx"`,
  `TARGETED_DEVICE_FAMILY = "1,2"` for iPhone and iPad, and the whole platform difference
  carried by per-SDK `INFOPLIST_FILE` and `CODE_SIGN_ENTITLEMENTS`.
  `SUPPORTS_MACCATALYST`, `SUPPORTS_MAC_DESIGNED_FOR_IPHONE_IPAD` and
  `SUPPORTS_XR_DESIGNED_FOR_IPHONE_IPAD` are all `NO`, and the first two for the same
  reason: the target already builds natively for macOS, so either would ship a second,
  worse Mac app. A second target would have duplicated all three build phases and the
  seven package product dependencies, and needed a second scheme, to express a difference
  that is four build settings. The sources were already shared, since `#if os(macOS)` has
  carried both layouts from the start.
- **`GENERATE_INFOPLIST_FILE = NO`**, unlike most apps here and for the same reason
  MNISTTrainer checks in its plist: `Info-macOS.plist` carries a literal
  `CFBundleName = "Shakespeare Reader"`, which is the app-menu title, and a generated
  plist would take the target name instead.
- **`Resources/Plays` is a folder reference, not 35 build files.** `CorpusLoader.load()`
  enumerates the directory, so a flattened copy would leave the app with no `Plays` to
  find. The `Applications` synchronized root group lists it under `explicitFolders`,
  which is what makes Xcode copy the directory whole to
  `$(UNLOCALIZED_RESOURCES_FOLDER_PATH)` rather than recursing into it. That is what
  preserves "adding a play is dropping a file in", which is the ethos that matters most
  here. Everything else the target compiles is named in its `membershipExceptions`, the
  same mechanism LLMEval and MLXChatExample use to claim their own subtrees of the shared
  group, so a new `.swift` file does need a project edit.
- **The icon needs no code.** `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon` is the whole
  of the wiring: actool writes `CFBundleIconName`, `CFBundleIconFile` and an
  `AppIcon.icns` into the bundle, and the Dock, Finder, Spotlight and ⌘-Tab read it from
  there without the process being consulted.
...
`Resources/Plays/NOTICE.md`, which also records the scene-0
decision behind the two Prologues.

## Icon provenance

The icon is original artwork, drawn by `tools/icon/make_icon.swift` from a font that
ships with macOS and two colours named in that file. There is no third-party image in
this directory and nothing about the icon to license, attribute or clear.
`tools/icon/NOTICE.md` records the design reasoning and why the mark is a letterform.
