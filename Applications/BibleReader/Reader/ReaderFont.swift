...
/// The face the scripture text is set in.
///
/// The app is otherwise built to imitate a printed edition — Challoner's notes set
/// smaller and indented under their verse, a verse number in a gutter, section
/// headings centred — and SF is the one part of it that does not read like a book.
///
...
enum ReaderFont: String, CaseIterable, Sendable {
    ...
    /// what decides whether `ReaderTypeface.noteItalic` can ask for `.italic()` or
    ...
    /// system face keeps each style's own metric curve, so the caption-derived section
...
/// The fonts the scripture text is actually drawn with.
///
/// **Roles, not styles.** One property per place that draws scripture, rather than a
...
/// it — and `VerseRow` stops restating styling it does not own.
struct ReaderTypeface: Equatable, Sendable {
    ...
    /// Probed once, here, rather than on every row: `noteItalic` is asked for at every
    /// note in the chapter.
    private let hasItalicFace: Bool

    /// The reader's size step, carried here rather than in a second environment key.
    ///
    /// `ChapterReaderView` re-anchors the scroll position off `.onChange(of: typeface)`,
    ...
    ///
    /// Stored here, alongside `textSize`, for the same reason: `ChapterReaderView`
    ...
    var chapterHeading: Font {
        ...
        guard let familyName else {
            ...
            // the point size, so a bare `Font.system(size:)` would draw the chapter
    ...
    var chapterTracking: CGFloat {
        (familyName == nil ? 0 : 0.8) * textSize.multiplier
    }

    /// Challoner's chapter argument, in the slot the scene setting occupied.
    var chapterArgument: Font {
        guard let familyName else {
            return textSize.isDefault
                ? .subheadline.italic() : system(.subheadline, italic: true)
        }
        guard hasItalicFace else { return Self.oblique(familyName, size: size(.subheadline)) }
        return custom(familyName, .subheadline).italic()
    }

    /// The Latin incipit of a psalm, which sits above the argument.
    var latinIncipit: Font {
        guard let familyName else {
            return textSize.isDefault ? .caption.italic() : system(.caption, italic: true)
        }
        guard hasItalicFace else { return Self.oblique(familyName, size: size(.caption)) }
        return custom(familyName, .caption).italic()
    }

    /// `ALEPH.`, `THE PARABLES OF SOLOMON`, `Alleluia.` — set as a small centred label,
    /// which is the slot the speaker heading occupied.
    ...
    /// The verse again, in italic.
    ///
    /// The transcription carries no `_..._` markup at all — that was checked over the
    /// whole 5.9 MB body, not sampled, and is why `GutenbergMarkup` does not exist in
    /// this app. What kept this role alive is Challoner: a note is printed as an italic
    /// **catchword** followed by upright commentary, so a note row has exactly one
    /// italic span and it is always its prefix. See `Row.italicSpans`.
    ///
    /// Body-sized and not callout-sized, which is what separates this from `note` below
    /// — an italic span is *in* the text, at the text's size.
    ...
    /// One of Challoner's notes: a step smaller than the verse, and **upright**, which
    /// is the one place this diverges from the stage direction it replaces. A direction
    /// is italic end to end; a note is upright commentary with an italic catchword, so
    /// the italic here would fight the only italic that carries meaning.
    var note: Font {
        guard let familyName else {
            return textSize.isDefault ? .callout : system(.callout)
        }
        return custom(familyName, .callout)
    }

    /// The note's catchword — the phrase from the verse that Challoner hangs the note
    /// on. Note-sized, so it sits in the run rather than above it.
    var noteItalic: Font {
        guard let familyName else {
    ...
    /// The gap *above* a section heading, tuned against 13pt SF. It is the one padding
    /// in `VerseRow` carrying typographic meaning, so it is the one that scales.
    var sectionGap: CGFloat { (6 * scale).rounded() }
    ...
    /// The section heading's inset, which lives here rather than in `VerseRow` because
    /// a printed edition sets it in ems: 28pt against 17pt type is not the same inset as
    /// 28pt against 26pt type.
    ///
    /// On **both** sides, because a section heading is centred and a long one should not
    /// centre across the whole measure.
    var sectionInset: CGFloat { (28 * scale).rounded() }

    /// A note's indent, on the leading side only. A note is subordinate to the verse
    /// above it, and an indent is how a printed edition says so.
    var noteInset: CGFloat { (24 * scale).rounded() }

    /// The verse-number gutter's font, which stays on the system face for its
    /// monospaced digits but does scale with the reader's step: a 10pt number beside
    /// 20pt scripture reads as a bug rather than as restraint.
    var gutterFont: Font {
    ...
    /// width is a budget for the widest label, so scaling the font without it is the
    /// clipping case.
    ///
    /// 38 rather than ShakespeareReader's 30, and the extra eight points are not for
    /// the three digits of Psalm 118:176. They are for the eighteen rows of Psalm 9's
    /// Hebrew-10 half, which this edition labels `9a:1` through `9a:18` — five
    /// characters, and the widest thing the gutter is ever asked to print.
    ///
    ...
    var gutterWidth: CGFloat { (38 * textSize.multiplier).rounded() }
    ...
    ///
    /// `scale` and not `textSize.multiplier`, for `sectionInset`'s reason restated: a
    ...
    /// **The source carries no line breaks, so this app does not invent any.**
    ///
    /// Psalms, Job, Proverbs, Ecclesiastes, Canticle of Canticles, Wisdom,
    /// Ecclesiasticus and Lamentations come through this transcription as wrapped
    /// prose, identical in shape to Genesis. Splitting a verse on its colons and
    /// semicolons to synthesize hemistichs would be a parser inventing a text, and it
    /// would be wrong exactly where a reader would notice.
    ///
    /// What is honest is typography: for a `.poetry` book, narrow the measure, loosen
    /// the leading, and inset the whole verse. That makes a verse read as a stanza
    /// rather than as a paragraph without asserting a single line break the source does
    /// not have. `Book.genre` is what selects it.
    ///
    /// A true turn-line hanging indent — continuation lines set further in than the
    /// first — is what a printed psalter does and is **not** implemented, because
    /// SwiftUI's `Text` does not honour an `NSParagraphStyle`'s `headIndent` and there
    /// is no other way to reach the wrapped lines of a single `Text`. The whole-verse
    /// inset below is the part of that convention that is reachable; the README records
    /// the gap.
    ///
    /// 480 against the 620 prose measure is roughly the ratio a printed Bible sets
    /// poetry to, and it is what keeps a psalm verse from running the full width.
    var poetryMeasure: CGFloat { (480 * scale).rounded() }

    /// Extra leading between the wrapped lines of a verse of poetry. Small on purpose:
    /// enough to open the block up, not enough to make one verse look like two.
    var poetryLineSpacing: CGFloat { (3 * scale).rounded() }

    /// How far a verse of poetry sits in from the prose margin.
    var poetryInset: CGFloat { (16 * scale).rounded() }

    // MARK: - Roles, resolved

    /// `verse`, `verseItalic` and `note` again, as concrete faces at concrete point
    ...
    /// `verseItalic`'s shadow, for the same reason `notePlatformFont` is `note`'s:
    /// italic advances are not the upright face's, so a span hit-tested with the
    /// upright font drifts from the word after it.
    ///
    ...
    /// A note, in the upright face it is drawn in.
    var notePlatformFont: PlatformFont {
        Self.face(familyName, size: scaledSize(.callout), italic: false)
    }

    /// A note's catchword, in the italic it is drawn in — italic advances are not the
    /// upright face's, so hit-testing the catchword with the upright one would drift
    /// every word of the note after it.
    ///
    /// Big Caslon is the exception and needs nothing special: `oblique(_:size:)` shears
    /// the matrix's `c` slot only, so its advances *are* the upright face's, and
    /// `hasItalicFace` is what keeps it out of the italic branch here.
    var noteItalicPlatformFont: PlatformFont {
        Self.face(
    ...
    /// and a note's catchword wraps exactly where its upright twin would. The size
...
extension EnvironmentValues {
    /// Injected on `ChapterReaderView` and nowhere else, and that single injection
