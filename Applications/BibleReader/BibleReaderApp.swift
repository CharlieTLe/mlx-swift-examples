...
struct AppOptions: Sendable {
    ...
    /// Print the assembled prompt and its exact token count for a sample of passages,
    /// then exit. Needs the tokenizer, so it does load the model.
    var showPrompt = false
    /// Run the real annotation path over a sample of passages and report the latency
    ...
    var modelID: String?
    /// Passages for `--show-prompt` and `--benchmark`, as
    /// `book:chapter:firstVerse-lastVerse`. Empty means the built-in sample.
    var passages: [String] = []
...
///
/// `--selftest` decodes the corpus and renders prompts, and never touches the GPU, so
/// it passes on a build whose Metal kernels were never compiled. mlx-swift's `.metal`
/// sources become a `default.metallib` inside `mlx-swift_Cmlx.bundle`, which is looked
/// up beside the running executable and only on first GPU use, where a miss surfaces
/// from C++ as "Failed to load the default metallib": long after launch, in the middle
/// of annotating a passage. Evaluating one array proves the kernels are there before a
/// user finds out the slow way, which is what makes this worth a flag of its own.
enum MetalCheck {
...
/// `BibleReaderApp.main()` is the way to get a `--selftest` that exits without ever
/// opening a window.
@main
enum EntryPoint {
    ...
    static func main() {
        ...
        // prompt dump or a benchmark table to. iOS parses the arguments and ignores
        // them, which leaves `--diagnostics` and `--model` working under
        // `xcrun simctl launch`.
        #if os(macOS)
        ...
        BibleReaderApp.options = options
        BibleReaderApp.main()
    }
}
    ...
    var body: some Scene {
        WindowGroup("Bible Reader") {
            ContentView(options: Self.options)
                ...
                // minimum. AppKit clamps an autosaved frame up to a raised minimum on
                // the next launch, so a window left narrower than this widens once and
                // holds.
                //
