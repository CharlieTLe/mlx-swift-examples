...
/// layout matches the first, which is what `ReaderTypeface.bodyPlatformFont` and its
/// two twins are for: same family, same point size, same wrap width, same italic runs,
...
/// - A near-miss is *visible*: `DocumentRowView` marks the word it resolved and the menu names
...
enum WordHitTest {
    ...
    ///
    /// It grew from a font and a width when the reader started drawing Gutenberg's
    /// italic spans and aligning directions the way Gutenberg's own edition does. Both
    /// additions are content-derived and constant for the life of a row, so a `Layout`
    /// is built per hit test and never cached.
    ///
    ...
    struct Layout {
        ...
        /// The face of an italic span. Inert on a direction line, which carries no
        /// spans at all because it is drawn italic end to end.
        var italicFont: PlatformFont
        /// UTF-16 ranges of the *stripped* text, from `GutenbergMarkup`.
        var italics: [Range<Int>] = []
        /// `.natural` for verse, so the shipped path stays the shipped path.
    ...
    /// character. The body `Text` spans the full pane width, so without that the mark
    ...
    ///
    /// The shipped path is kept byte for byte for a line with no spans and no
    /// alignment — no paragraph style at all, which is CoreText's own default — so the
    /// verse, which is every line but the directions and the ~900 with an italic span
    /// in them, lays out exactly as it always has.
    private static func layout(_ text: String, _ spec: Layout) -> [(CTLine, CGPoint)]? {
        ...
        if spec.italics.isEmpty, spec.alignment == .natural {
            ...
            for span in spec.italics {
                // Clamped rather than trusted: the spans are measured against
                // `plainText` and so are always in range, but a range past the end is a
