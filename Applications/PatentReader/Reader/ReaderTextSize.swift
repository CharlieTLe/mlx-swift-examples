...
/// How large the patent text is set, as a multiplier over the system's own point size
/// for each text style.
///
...
enum ReaderTextSize: String, CaseIterable, Sendable {
    ...
    /// this, so a neutral step that is merely close would move the text for every
