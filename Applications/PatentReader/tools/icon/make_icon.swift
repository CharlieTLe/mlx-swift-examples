...
//
// Regenerate the app icon: a Big Caslon capital P, ink on parchment.
//
//     swift tools/icon/make_icon.swift
//
// No shebang, unlike tools/wire_target.py: swift-format treats one as a comment and
// folds the following line into it, so a `#!/usr/bin/swift` here comes back mangled from
// every `pre-commit run --all`. Invoked through `swift` instead, which is how the wiring
// script is invoked through `python3` anyway.
//
// Writes Assets.xcassets/AppIcon.appiconset/*.png: the macOS ladder plus the iOS 1024.
// Every output is checked in, so a clone builds with no run of this script; it exists to
// make the mark and the masking reproducible, the same way tools/wire_target.py does for
// the target's entry in the project file.
//
...
// deliberately: wire_target.py is standard-library-only so it needs no install step, and
// the equivalent image script would need Pillow. CoreGraphics is already on every machine
// that can build this app, it is what ships the high-quality downsampling the 16pt icon
// needs, and CoreText is what turns a glyph into a resolution-independent path.
...
///
/// Big Caslon is one of the faces the reader offers, and it is the only Caslon macOS
/// ships, so the icon is set in something the app actually renders patent text in rather
/// than in a face chosen only for the icon. `BigCaslon-Medium` is its single weight;
/// there is no bold or italic, which is why `markFontName` is a PostScript name and is
/// checked rather than requested by family.
let markCharacter: UniChar = 0x0050  // "P"
let markFontName = "BigCaslon-Medium"

/// Height of the glyph's own bounding box as a fraction of the art canvas.
///
/// A `P` is narrow, flat-topped and sits on the baseline, so its bounding box *is* the cap
/// height — where a round letter's overshoots at both ends make the box taller than the
/// cap it encloses. So this is set higher than the 0.62 that suits ShakespeareReader's
/// `S`: at that value the same nominal fraction buys a shorter cap, and the narrow
/// silhouette leaves the tile looking half empty besides. Rendered and compared at 0.62,
/// 0.65, 0.68 and 0.72 — by 0.72 the top and foot serifs crowd the squircle's edge, and
/// 0.65 is the value that gives this letter the presence the `S` has at 0.62.
let markHeightFraction = 0.65
...
let appRoot = toolURL.deletingLastPathComponent()  // tools/icon
    ...
    .deletingLastPathComponent()  // PatentReader
...
/// scaled to an exact height and centred on its own bounding box. Laying it out as text
/// instead would centre the *typographic* box — advance width, ascent and descent — and a
/// `P` has no descender, so the letter would sit visibly high in the tile. The horizontal
/// centring is worth as much: a Caslon `P` hangs a wide bowl off the right of a straight
/// stem and carries a left sidebearing the right side has no answer to, so its ink is not
/// centred in the advance width either.
func markArt(size: Int) -> CGImage {
    ...
    // Scale to the target height, then translate the *scaled* bounding box to the centre
    // of the canvas. Order matters: scaling after the translate would move the letter off
    // centre by the scale factor.
